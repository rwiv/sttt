...
class Transcriber:
    def __init__(
        ...
        self.__word_threshold = word_threshold
    ...
    def transcribe(self, segments: list[Segment]) -> list[Sentence]:
        if self.__seg_relocation:
            return self.__relocate_words2(segments)
        else:
            return [Sentence(start=s.start, end=s.end, text=s.text.strip()) for s in segments]

    def __relocate_words2(self, segments: list[Segment]) -> list[Sentence]:
        words: list[Word] = []
        for seg in segments:
        ...
        for word in words:
            ...
            if word.check_last() or word.text.strip().endswith((".", "?", "!")):
                sentences.append(merge_words(tmp_words))
                tmp_words = []
                continue

            if word.text.strip().endswith((",", ":", ";")) and len(tmp_words) > self.__word_threshold:
                sentences.append(merge_words(tmp_words))
                tmp_words = []
                continue
